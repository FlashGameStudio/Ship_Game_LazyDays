...
public class GraphSearch
{
    ...
    /// <summary>
    /// Returns whole map of hexes (atm 600)
    /// </summary>
    /// <param name="hexGrid"></param>
    /// <param name="startPoint"></param>
    /// <returns></returns>
    public static BFSResult BFSGetRangeForNPC(HexGrid hexGrid, Vector3Int startPoint)
    {
        //Debug.Log($"hex grid contains {hexGrid.map.Count()} map parts");
        Dictionary<Vector3Int, Vector3Int?> visitedNodes = new Dictionary<Vector3Int, Vector3Int?>();
        Dictionary<Vector3Int, int> costSoFar = new Dictionary<Vector3Int, int>();
        Queue<Vector3Int> nodesToVisitQueue = new Queue<Vector3Int>();
        ...
        costSoFar.Add(startPoint, 0);
        visitedNodes.Add(startPoint, null);
        //Debug.Log($"nodes to visit:{nodesToVisitQueue.Count}"); 
        while (nodesToVisitQueue.Count > 0)
        {
            Vector3Int currentNode = nodesToVisitQueue.Dequeue();
            foreach (Vector3Int neighbourPosition in hexGrid.GetNeighboursFor(currentNode))
            {
                ...
                int nodeCost = 0;// hexGrid.GetTileAt(neighbourPosition).GetCost();
                int currentCost = costSoFar[currentNode];
                int newCost = currentCost + nodeCost;
        ...
        //Debug.Log($"nodes to visit:{visitedNodes.Count}");
        return new BFSResult { visitedNodesDict = visitedNodes };
    }
